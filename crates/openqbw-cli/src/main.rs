...
use rusqlite::{Connection, MAIN_DB, Transaction, params};
#[cfg(feature = "research-tools")]
...
fn accounting_blocker_summary(
    ...
    for blocker in blockers {
        let label = match blocker {
            ...
            openqbw::Enterprise24AccountingPipelineBlocker::DuplicateTableCoverage { table_id } => {
                format!("duplicate-table-coverage-{table_id}")
            }
            openqbw::Enterprise24AccountingPipelineBlocker::TablePolicyCoverageMismatch {
                policy_table_id,
                coverage_table_id,
            } => format!("table-policy-coverage-mismatch-{policy_table_id}-{coverage_table_id}"),
            openqbw::Enterprise24AccountingPipelineBlocker::DuplicateAccountRecordNumber {
                ..
            } => "duplicate-account-record-number".to_owned(),
...
enum Cmd {
    ...
    Export {
        /// Input QBW file.
        input: PathBuf,
        /// Output SQLite database. Existing files are refused unless --force is supplied.
        output: PathBuf,
        /// Replace an existing output only after a complete replacement is staged.
        #[arg(long)]
        force: bool,
    },
    ...
    ReconcileTrialBalance {
        ...
        actual: PathBuf,
        /// Print account names and variances on failure. These details can be sensitive.
        #[arg(long)]
    ...
    AccountingReport {
        ...
        /// New output file. Existing files are refused unless --force is supplied.
        #[arg(long)]
        out: PathBuf,
        /// Replace an existing output only after a complete replacement is staged.
        #[arg(long)]
...
fn main() -> Result<()> {
    ...
    match cli.cmd {
        Cmd::Export {
            ...
            output,
            force,
        } => run_export(input, output, force).map(|_| ()),
        ...
        Cmd::Migrate {
            ...
            format,
            force,
        } => run_migrate(input, out, format, force),
        Cmd::Forensics { input } => run_forensics(input),
        Cmd::ReconcileTrialBalance {
            reference,
            actual,
            show_account_details,
        } => run_reconcile_trial_balance(reference, actual, show_account_details),
...
fn run_batch_trial_balance_command(
    ...
    run_batch_trial_balance(
        ...
        &generated_at,
        &out,
        build_local_enterprise24_ledger_from_bytes,
    )
...
fn run_accounting_report(
    ...
    ensure_safe_destination(&input, &out, force)?;
...
fn write_new_report(path: &Path, contents: &str, force: bool, input: &Path) -> Result<()> {
    write_staged_file(path, force, Some(input), |_staged_path, mut file| {
        file.write_all(contents.as_bytes())
            .with_context(|| "writing staged report output")
    })
}
...
fn write_report_sqlite(
    ...
    write_staged_file(path, force, Some(input), |_staged_path, mut file| {
        let mut connection =
            Connection::open_in_memory().context("opening in-memory SQLite report")?;
        write_sqlite_with_account_catalog(&mut connection, metadata, bundle, account_catalog)
            .map_err(anyhow::Error::msg)?;
        let serialized = connection
            .serialize(MAIN_DB)
            .context("serializing staged SQLite report")?;
        file.write_all(&serialized)
            .context("writing serialized staged SQLite report")?;
        file.sync_all().context("syncing staged SQLite report")
    })
}
...
fn build_local_enterprise24_ledger(
    ...
    build_local_enterprise24_ledger_from_store(&store, snapshot_id)
}

fn build_local_enterprise24_ledger_from_bytes(
    bytes: Vec<u8>,
    snapshot_id: String,
) -> Result<openqbw::Ledger> {
    let store = PageStore::from_bytes(bytes).context("opening local QBW input")?;
    build_local_enterprise24_ledger_from_store(&store, snapshot_id)
}

fn build_local_enterprise24_ledger_from_store(
    store: &PageStore,
    snapshot_id: String,
) -> Result<openqbw::Ledger> {
...
fn run_reconcile_trial_balance(
    reference: PathBuf,
    actual: PathBuf,
    show_account_details: bool,
) -> Result<()> {
    ...
    run_trial_balance_reconciliation(&reference_tb, &actual_tb, show_account_details)
}
...
fn run_trial_balance_reconciliation(
    ...
    finish_trial_balance_reconciliation(reference_tb, actual_tb, result, show_account_details)
}
...
fn finish_trial_balance_reconciliation(
    ...
    if !result.passes() {
        if show_account_details {
            ...
            eprintln!(
                "account-level reconciliation details suppressed; rerun with --show-account-details to display local account names and variances"
            );
        }
...
/// Output is always built beside its final destination. This keeps publication
/// on one filesystem, so rename is atomic once the complete artifact exists.
fn staged_output_path(destination: &Path) -> Result<PathBuf> {
    let parent = destination.parent().unwrap_or_else(|| Path::new("."));
    if !parent.is_dir() {
        anyhow::bail!("output parent directory does not exist");
    }
    for _ in 0..128 {
        let n = STAGED_OUTPUT_COUNTER.fetch_add(1, Ordering::Relaxed);
        let candidate = parent.join(format!(".openqbw-stage-{}-{n}", std::process::id()));
        if !candidate.exists() {
            return Ok(candidate);
        }
    }
    anyhow::bail!("could not reserve a unique staged output path")
}

fn create_staged_file(destination: &Path) -> Result<(tempfile::TempDir, PathBuf, std::fs::File)> {
    let parent = destination.parent().unwrap_or_else(|| Path::new("."));
    let directory = tempfile::Builder::new()
        .prefix(".openqbw-private-stage-")
        .tempdir_in(parent)
        .context("creating private staged-output directory")?;
    let path = directory.path().join("artifact");
    let file = std::fs::OpenOptions::new()
        .write(true)
        .create_new(true)
        .open(&path)
        .context("creating staged output")?;
    Ok((directory, path, file))
}

fn create_staged_directory(destination: &Path) -> Result<(tempfile::TempDir, PathBuf)> {
    let parent = destination.parent().unwrap_or_else(|| Path::new("."));
    let directory = tempfile::Builder::new()
        .prefix(".openqbw-private-stage-")
        .tempdir_in(parent)
        .context("creating private staged-output directory")?;
    let path = directory.path().join("artifact");
    std::fs::create_dir(&path).context("creating staged output directory")?;
    Ok((directory, path))
}
...
fn same_file(left: &Path, right: &Path) -> Result<bool> {
    ...
    same_file::is_same_file(left, right).context("comparing input and output file identities")
}

/// Call only after the source has been successfully opened/validated. It
/// rejects lexical/canonical aliases as well as hard links before any output
/// mutation can occur.
fn ensure_safe_destination(input: &Path, destination: &Path, force: bool) -> Result<()> {
    if same_file(input, destination)? {
        anyhow::bail!("refusing output that aliases the input file");
    }
    if destination.exists() && !force {
        anyhow::bail!("refusing to overwrite existing output; pass --force after reviewing it");
    }
    Ok(())
}

fn remove_staged_path(path: &Path, is_directory: bool) {
...
/// When replacement is explicit, preserve the old destination as a sibling
/// backup. The backup remains after successful publication for explicit review
/// and cleanup; this function never deletes a path that could have been swapped
...
fn publish_staged_path(
    ...
    force: bool,
    is_directory: bool,
    protected_input: Option<&Path>,
) -> Result<()> {
    if let Some(input) = protected_input {
        if !input.exists() {
            remove_staged_path(stage, is_directory);
            anyhow::bail!("input disappeared before output publication");
        }
        if same_file(input, destination)? {
            remove_staged_path(stage, is_directory);
            anyhow::bail!("refusing output that aliases the input file at publication time");
        }
    }
    let backup = if destination.exists() {
        ...
        let backup = staged_output_path(destination)?;
        std::fs::rename(destination, &backup)
            .context("preserving existing output before replacement")?;
        Some(backup)
    } else {
        None
    };
    if let (Some(input), Some(backup_path)) = (protected_input, backup.as_deref())
        && (!input.exists() || same_file(input, backup_path)?)
    {
        if !destination.exists() {
            let _ = std::fs::rename(backup_path, destination);
        }
        remove_staged_path(stage, is_directory);
        anyhow::bail!("destination changed while output publication was starting");
    }
    let publish_result = if is_directory {
        std::fs::rename(stage, destination)
    } else {
        std::fs::hard_link(stage, destination).and_then(|()| std::fs::remove_file(stage))
    };
    match publish_result {
        Ok(()) => {
            if backup.is_some() {
                eprintln!(
                    "previous output preserved in a uniquely named sibling backup; remove it only after reviewing the replacement"
                );
            }
            Ok(())
        }
        Err(error) => {
            ...
            remove_staged_path(stage, is_directory);
            Err(error).context("publishing staged output")
        }
    }
}

fn write_staged_file<T>(
    destination: &Path,
    force: bool,
    protected_input: Option<&Path>,
    write: impl FnOnce(&Path, std::fs::File) -> Result<T>,
) -> Result<T> {
    let (_stage_directory, stage, file) = create_staged_file(destination)?;
    let result = write(&stage, file);
    match result {
        Ok(value) => {
            publish_staged_path(&stage, destination, force, false, protected_input)?;
            Ok(value)
        }
        Err(error) => {
...
fn run_export(input: PathBuf, output: PathBuf, force: bool) -> Result<ExportStats> {
    ...
    ensure_safe_destination(&input, &output, force)?;
    let stats = write_staged_file(&output, force, Some(&input), |_staged_path, mut file| {
        let mut conn = Connection::open_in_memory().context("opening in-memory SQLite output")?;
        create_schema(&conn)?;
        {
        ...
        let serialized = conn
            .serialize(MAIN_DB)
            .context("serializing staged SQLite output")?;
        file.write_all(&serialized)
            .context("writing serialized staged SQLite output")?;
        file.sync_all().context("syncing staged SQLite output")?;
        Ok(stats)
    })?;
...
fn run_verify(input: PathBuf, output: Option<PathBuf>, strict_attribution: bool) -> Result<()> {
    ...
    let stats = run_export(input.clone(), out.clone(), false)?;
    println!();
...
fn run_migrate(input: PathBuf, out: PathBuf, format: MigrateFormat, force: bool) -> Result<()> {
    match format {
        MigrateFormat::Sqlite => run_export(input, out, force).map(|s| println!("{}", s.summary())),
        MigrateFormat::Csv => run_migrate_csv(input, out, force),
        MigrateFormat::Iif => run_migrate_iif(input, out, force),
    }
}
...
fn run_migrate_csv(input: PathBuf, out: PathBuf, force: bool) -> Result<()> {
    let (items, headers, store, model) = collect_records(&input)?;
    ensure_safe_destination(&input, &out, force)?;
    let (_stage_directory, stage) = create_staged_directory(&out)?;
    let mut tables: Vec<SysTableEntry> = openqbw::collect_unique(&store, &model);
    tables.sort_by_key(|e| e.table_id);

    // catalog.csv
    let write_result = (|| -> Result<()> {
        let mut cat = std::fs::File::create(stage.join("catalog.csv"))?;
        use std::io::Write;
        writeln!(
            cat,
        ...
        // transactions.csv
        let mut txf = std::fs::File::create(stage.join("transactions.csv"))?;
        ...
        // lineitems.csv
        let mut lif = std::fs::File::create(stage.join("lineitems.csv"))?;
        writeln!(
            lif,
    ...
    publish_staged_path(&stage, &out, force, true, Some(&input))?;
    println!(
...
fn run_migrate_iif(input: PathBuf, out: PathBuf, force: bool) -> Result<()> {
    let (items, headers, _store, _model) = collect_records(&input)?;
    ensure_safe_destination(&input, &out, force)?;
    use std::io::Write;
    let (_stage_directory, stage, mut f) = create_staged_file(&out)?;
    ...
    drop(f);
    publish_staged_path(&stage, &out, force, false, Some(&input))?;
    println!(
...
mod accounting_routing_tests {
    ...
    fn output_test_directory(label: &str) -> PathBuf {
        let sequence = STAGED_OUTPUT_COUNTER.fetch_add(1, Ordering::Relaxed);
        let path = std::env::temp_dir().join(format!(
            "openqbw-cli-output-test-{label}-{}-{sequence}",
            std::process::id()
        ));
        std::fs::create_dir(&path).expect("create synthetic test directory");
        path
    }

    #[test]
    fn output_guard_rejects_exact_relative_canonical_and_hardlink_input_aliases() {
        let directory = output_test_directory("aliases");
        let input = directory.join("synthetic-input.qbw");
        std::fs::write(&input, b"synthetic only").expect("write synthetic input");
        assert!(ensure_safe_destination(&input, &input, false).is_err());
        assert!(
            ensure_safe_destination(
                &input,
                &directory.join(".").join("synthetic-input.qbw"),
                false
            )
            .is_err()
        );
        let hardlink = directory.join("synthetic-hardlink.qbw");
        std::fs::hard_link(&input, &hardlink).expect("create synthetic hard link");
        assert!(ensure_safe_destination(&input, &hardlink, true).is_err());
        std::fs::remove_dir_all(directory).expect("remove synthetic test directory");
    }

    #[test]
    fn publication_rechecks_an_input_alias_created_after_initial_validation() {
        let directory = output_test_directory("publication-race");
        let input = directory.join("synthetic-input.qbw");
        let output = directory.join("existing.sqlite");
        std::fs::write(&input, b"synthetic input").expect("write synthetic input");
        std::fs::write(&output, b"old artifact").expect("write old artifact");
        ensure_safe_destination(&input, &output, true).expect("initial destination is distinct");

        let (_stage_directory, stage, mut file) =
            create_staged_file(&output).expect("create staged output");
        file.write_all(b"replacement").expect("write staged output");
        drop(file);
        std::fs::remove_file(&output).expect("simulate destination replacement");
        std::fs::hard_link(&input, &output).expect("replace destination with input alias");

        assert!(publish_staged_path(&stage, &output, true, false, Some(&input)).is_err());
        assert_eq!(
            std::fs::read(&input).expect("read preserved input"),
            b"synthetic input"
        );
        assert_eq!(
            std::fs::read(&output).expect("read preserved alias"),
            b"synthetic input"
        );
        assert!(!stage.exists());
        std::fs::remove_dir_all(directory).expect("remove synthetic test directory");
    ...
    #[test]
    fn failed_staged_write_preserves_existing_output() {
        let directory = output_test_directory("preserve");
        let output = directory.join("existing.sqlite");
        std::fs::write(&output, b"old artifact").expect("write existing artifact");
        let result: Result<()> = write_staged_file(&output, true, None, |_stage, mut file| {
    ...
    #[test]
    fn forced_publication_retains_the_prior_artifact_as_a_backup() {
        let directory = output_test_directory("force-backup");
        let output = directory.join("existing.sqlite");
        std::fs::write(&output, b"old artifact").expect("write existing artifact");
        write_staged_file(&output, true, None, |_stage, mut file| {
